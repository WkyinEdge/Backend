        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <span v-if="typeof(v.value)=='string'">
              <span v-if="v.type == 'image'"> <img :src="scope.row[v.value]" :alt="scope.row[v.value]" width="100%"></span>
            ...
            <span v-else-if="typeof(v.value)=='object'">
              <span v-for="(v1,k1) in v.value" :key="k1">
                ...
                <span class="more-info-display" v-else-if="scope.row[v1.value] || scope.row[v1.value_alias] || scope.row[v1]" >
                  ...
                  <span class="is-label" v-if="typeof(v1)=='object' && v1.label"  v-html="v1.label"><!--:style="{width:v1.width?parseInt(v1.width)+'px':'72px'}"--></span>
