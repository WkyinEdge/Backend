      ...
      <el-tab-pane label="账号密码登录" name="login1">
        <el-form ref="loginForm" :model="loginData" :rules="rules">
          <el-form-item prop="account">
            <el-input v-model="loginData.account" placeholder="账号：wky" prefix-icon="iconfont icon-shouji4"></el-input>
          </el-form-item>
          <el-form-item prop="password">
            <el-input type="password" v-model="loginData.password" placeholder="密码：123" prefix-icon="iconfont icon-lock"></el-input>
          </el-form-item>
          <el-form-item>
...
export default {
  ...
  methods: {
    ...
    //同步时间
    async syncTime(){
      let serverTime = await this.$api.syncTime()
      serverTime = parseInt(serverTime)
      let bdTime = Date.now() / 1000;
      let timec = parseInt( serverTime - bdTime )
      localStorage.setItem('syncTime',timec)
    },
    // 路由跳转
    go(url='/') {
