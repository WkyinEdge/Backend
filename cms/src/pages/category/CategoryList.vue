...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        name: [{required: true, message: '输入分类名称', trigger: 'blur' }],
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    async deleteButton(id) {
      ...
      await this.$api.deleteCategoryInfo(id)
    ...
    async formSubmit() {
      let ids = this.$refs['treeTable'].getExpandIds()
      await this.$api.saveCategory(this.formData)
      let list= await this.$api.getCategoryList()
      this.categoryList=list
    ...
    async dragSortSubmit(orderIds) {
      let ids = this.$refs['treeTable'].getExpandIds()
      await this.$api.orderCategory(orderIds)
