...
export default {
  ...
  methods: {
    ...
    async switchPaging() {
      ...
      let res = await this.$api.getRolesList(pagingInfo)
      let list = res.list, pages = res.pages
    ...
    async editButton(id) {
      ...
      let res = await this.$api.getRolesInfo(id)
      this.formData = res
      this.dialogData.loading = false
    },
    // form数据提交，请求接口
    async formSubmit() {
      let res = await this.$api.saveRoles(this.formData)
        res = await this.$api.getRolesList()
    ...
    async deleteButton(id) {
      let res = await this.$api.deleteRoles(id)
        res = await this.$api.getRolesList()
        this.rolesList = res.list
    ...
    async permissionsBinding(row) {
      ...
      let res = await this.$api.getRolesPermissions(row.id)

      this.permissionsList = res.permissions_list
    ...
    async drawerConfirm() {
      ...
      let params = {
        ...
        permissions_id: JSON.stringify(this.rolePermissions)
      }
  ...
  async mounted() {
    ...
    let res = await this.$api.getRolesList(pagingInfo)
    this.rolesList = res.list
    this.pagingData.total = res.pages.total
    this.loadingStaus=false
  },
}
</script>
