...
<!--                <el-select v-model="formData.user_roles" multiple filterable clearable placeholder="请选择">-->
                <el-select v-model="formData.role_id" filterable clearable placeholder="请选择">
                  <el-option
...
export default {
  ...
  methods: {
    ...
    async addButton() {
      ...
      let res = await this.$api.getUserRoles()
      this.rolesList = res
      this.$nextTick(() => {
    ...
    async editButton(id) {
      ...
      let res = await this.$api.getUserRoles()
      this.rolesList = res
      res = await this.$api.getUserInfo(id)
      this.formData = res
      this.drawerData.loading = false
    },
    // 处理抽屉关闭
    ...
    async formSubmit() {
      ...
      this.drawerData.loading = true
      let res = await this.$api.saveUser(this.formData)
      if (res){
        this.$nextTick(() => {
          this.drawerData.visible = false
        })
    ...
    async initUserList(type) {
      ...
      let res = await this.$api.getUserList(searchCondition)
      this.userList = res.list
      this.pagingData.total = res.pages.total
      this.offset = res.pages.offset
