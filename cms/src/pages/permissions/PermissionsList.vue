    ...
    <PageHeaderLayout>
      <div class="main-page-content">
        <el-row class="table-header">
          ...
          <el-cascader  v-if="userPermissions.indexOf('permissions_edit') != -1"
                        placeholder="选择显示类型"
                        size="medium"
                        :options="typeList"
        ...
        <TreeTable :data="permissionsList" :columns="columns" :loading="loadingStaus" ref="treeTable" highlight-current-row border>
          ...
          label="操作">
            <template slot-scope="scope">
              <el-tooltip effect="dark" content="添加子权限" placement="top-start"  v-if="userPermissions.indexOf('permissions_create') != -1 && scope.row.parent_id == 0" >
          ...
          label="操作">
            <template slot-scope="scope">
              <el-button size="mini" icon="iconfont" v-if="userPermissions.indexOf('permissions_create') != -1 && scope.row.parent_id == 0" @click="addButton(scope.row.id,scope.row.name)">添加子权限</el-button>
...
export default {
  ...
  methods: {
    ...
    async editButton(id,parent) {
      ...
      let res = await this.$api.getPermissionsInfo(id)
      this.formData = res
      // 编辑子权限，拼接的前缀
      if (parent) {
    ...
    async formSubmit() {
      ...
      let res = await this.$api.savePermissions(this.formData)
      if(res){
        res = await this.$api.getPermissionsList()
    ...
    async deleteButton(id) {
      let ids = this.$refs['treeTable'].getExpandIds()
      let res = await this.$api.deletePermissions(id)
      if (res){
        res = await this.$api.getPermissionsList()
        this.permissionsList = res
        this.$nextTick(() => {
    ...
    async dragSortSubmit(orderIds) {
      let ids = this.$refs['treeTable'].getExpandIds()
      let res = await this.$api.orderPermissions(orderIds)
      if (res){
        res = await this.$api.getPermissionsList()
    ...
    async typeChange(v) {
      if (v.length) {
        ...
        let res = await this.$api.getPermissionsList({status:state})
        this.permissionsList = res ? res : []
      }
    },
  },
  async mounted() {
    let res = await this.$api.getPermissionsList()
    this.permissionsList = res ? res : []
